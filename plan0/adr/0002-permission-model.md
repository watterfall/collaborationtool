...
## Phase 5 ADR-0020 Triadic 联动：4 角色 + 新 capability vocabulary（2026-05-12）

ADR-0020 §2.4 引入 4 角色分化（Explorer / Bridge-builder / Validator /
Connector）。Wave D-1 `2faefe2` 在 `packages/discovery-graph/src/role.ts`
落地 `ROLES` enum + `DEFAULT_SURFACE_BY_ROLE`（/discover /translate
/manuscript /network）。

**与本 ADR §2.2 5 default role bundle 的关系**：

- ADR-0020 4 角色是 **UI surface 偏好**，不是 RBAC role bundle。Wave D-4
  `3aa4f57` 把角色映射到 `/triadic/{discover,translate,manuscript,network}`
  surface，没动 capability 检查路径。
- 既有 5 role bundle（owner / editor / commenter / reviewer / viewer）继续
  作权限单元；4 角色是 orthogonal cut（一个 user 既是 commenter 又可以
  是 Connector）。
- 本 ADR §2.2 vocabulary **现未新增**——Night/Bridge artifact 操作 Wave D
  阶段都 type-only，无 PG 写路径。Phase 6 follow-up ADR（若 ADR-0020 →
  Accepted）才落 `night.artifact:create` / `bridge.artifact:create` /
  `cross-layer-reference:create` 等动词，那时再加 §2.6 review log。
- ContributionGraph attribution 反 priority race（discovery-graph
  `contribution-graph.ts`）—— 14 ContributionKind 是 **artifact metadata**，
  不接 capability 检查；attribution 不等同 ACL grant。

**不开新 ADR**：4 角色是 ADR-0020 内决策，本 ADR 仅登记 RBAC vs UI-role
正交切分。Wave D-5 dogfood gate 通过后若需新 capability，按 §4.1 ADR-driven
流程开 follow-up。
