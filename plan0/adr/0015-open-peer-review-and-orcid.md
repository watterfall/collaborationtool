...
### 7.4 Phase 6 W2 ADR-0018 — open project / open question via ORCID（2026-05-12）

ADR-0018 Open Content Mechanisms（`b3df724` Proposed + `7e6c730` schema +
`71c8228` F4 publish route）引入 4 个 first-class open entity，**全部
ORCID-signed**（per ADR-0018 §2.5 invariant：open_peer_review.reviewer_orcid_id
required；anonymous reviews 不入 Merkle log，仅在 comment-store 旁路）。

**对本 ADR ORCID OAuth flow 的影响**：复用，不重写。better-auth
genericOAuth 配置 + JWS detached signature pattern 与本 ADR §2.2-§2.4
同源；ADR-0018 §2.4 F4 publish flow 借用 ORCID JWS 作为 signed_payload_jws
载体（payload 是 canonical entity content + content_hash）。

**对 review.signed_payload_jws 的影响**：claim_review 表（ADR-0016）签名
模式与 ADR-0018 open_peer_review 表签名模式**完全同源**——同样的 JWS
detached signature + ORCID iD link + withdraw mark-only。Phase 6 W3+ 加
`principal.ed25519_public_key` PG 列后，server-side signature verify 真
落地（本 ADR §3.2 风险预警的 JWKS cache 策略与 ADR-0018 共享）。

**新增数据流 F7 open question stranger reply**：本 ADR §5 open questions
（待 Phase 4 W8 dogfood gate）+ ADR-0018 §2.5 F7 lifecycle 结合 ——
ORCID-authed stranger 通过 web inline editor 回复 open_question →
open_peer_review (target_kind='question') → Merkle log entry + signed
JWS → desktop owner pull (per ADR-0017 visibility routing)。

**Evidence Tier**：本 ADR ORCID OAuth `contract` (env 未配 ghost-disabled
fallback) → Phase 6 W2-W3 真 sandbox.orcid.org OAuth 跑通后升 `real`。
