...
| 0008 | Long-horizon agent runtime + reviewer/researcher agent | **Accepted (with caveat)** | mixed | agent_job + agent_job_event 表 + apps/agent-worker pgboss subscribe stub 全 PASS（`contract`）；reviewer/researcher path 仍 `markDone` stub（`apps/agent-worker/src/index.ts:132-149` `mock`）；**Phase 5 Wave A A1（2026-05-11）quota enforce `real`**：migration 0013 落 `agent.quota_per_day` + `agent_invocation_log` 表；`packages/ai-runtime/src/quota-enforcer.ts` PG counter + rolling 24h window + (principal,kind) 分区；`/api/agent/invoke` 前置 + agent-worker `handleOne` 前置防御（超 quota 429 + agent_job markError + agent_job_event{kind:'error', errorClass:'quota-exceeded'}）；11 单元测试覆盖；cancel route 仍 `mock`（A2 待落，POST /api/agent/job/<jobId>/cancel + worker poll cancelling）；W9 G3 multi-agent real LLM round-trip skip-by-env（spec 在但跑 `mock`）；caveat: cancel API + worker polling timeout + reviewer/researcher 真 LLM round-trip 全 `real` 推 Phase 5 Wave A A2-A4 |
