...
| 0008 | Long-horizon agent runtime + reviewer/researcher agent | **Accepted (with caveat)** | mock | agent_job + agent_job_event 表 + apps/agent-worker pgboss subscribe stub 全 PASS（`contract`）；reviewer/researcher path 仍 `markDone` stub（`apps/agent-worker/src/index.ts:132-149` `mock`），quota parse `contract`，quota enforce + cancel route `mock`；W9 G3 multi-agent real LLM round-trip skip-by-env（spec 在但跑 `mock`）；caveat: quota enforcer + cancel API + worker polling timeout + reviewer/researcher 真 LLM round-trip 全 `real` 落 Phase 5 Wave A（**codex review 2026-05-11 §2 标 P0**） |
