...
**Phase 4 推迟项**：W1 dogfood gate（bwrap 真启动 require Linux host）+ W2 dogfood gate（4 endpoint round-trip require API key）+ W3 端到端真 multi-agent 跑通 + W4 dogfood（pgboss queue 真启 + 6 finding 各 1 fixture 跑通 + dashboard 实测）+ W5-W6 Yjs subdocument 实施 + W7 章节 fork-merge UI + W8 ORCID 真集成 + W9 跨设备同步 + W10 Loro / Automerge 3 review。
