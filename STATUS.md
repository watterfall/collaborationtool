...
| 0011 | Claim/Evidence/Counterpoint/Synthesis 一等知识对象层 | **Accepted** | W5 schema + PM 节点 + W7 Evidence Map + W4 6 finding 生成器全交付 + W9 G4 dogfood gate sandbox 全 PASS；§7 review log 写 W7 + W9 dogfood criteria |
| 0012 | Plugin sandbox + 用户安装路径 + capability 提示 UI | **Accepted (with caveat)** | 起草 → Proposed → 2026-05-11 Accepted with caveat；Phase 4 W1 backend + 13 测；W4 W1 dogfood UI（capability prompt + sandbox descriptor）；**W8** 非 Linux 平台 UI 显式拦截（apps/web settings/plugins）+ URL/paste 双轨 + Provenance card capability prompt 风格；**W9 G1 CI workflow 就绪**（dogfood-bwrap.yml）；caveat: bwrap 真启动 require ubuntu-latest CI runner secrets 配置 + macOS sandbox-exec 真写推 Phase 5 W1（当前 UI 拦截兜底） |
| 0013 | ModelProvider abstraction + BYO 模型 + 配置存储 | **Accepted (with caveat)** | 起草 → Proposed → 2026-05-11 Accepted with caveat；Phase 3 4 adapter + 11 测 + Phase 4 W2 resolver 4 档 + 9 测；**W7.2 plugin contract `anthropic → provider: ModelProvider` 全切**（types.ts + 6 plugin agent.ts；Anthropic SDK 调用次数 = 0；ai-runtime 105→111 测）；**W9 G2 CI matrix 就绪**（dogfood-providers.yml × 4 wire）；caveat: 4 endpoint 真 round-trip require API key secrets（ANTHROPIC / OPENAI / OLLAMA / CUSTOM_HTTP）|
| 0014 | Yjs subdocument 章节级拆分 + cross-reference sync | **Accepted (with caveat)** | 起草 → Proposed → 2026-05-11 Accepted with caveat；W5 schema + W7.1 doc-store 抽象（DocumentHandle + YjsDocumentHandle + DocStore 缓存 + subdocument helper，17 测）；§5 dual-write owner 决策 = `apps/snapshot-worker`，W9 落 cross-ref-sync.ts（11 测：Y.Map.observe → PG crossref_index 增量 + reconcile 启动收敛）；**W9 G5 stress 5-25 client + cross-ref-sync 单测全 PASS**；caveat: 50 客户端并发 + 真多 subdoc Yjs 挂载 require sync-gateway 多 subdoc 路由（W5-W6 + Phase 5）|
| 0015 | Open peer review + ORCID-signed reviews | **Accepted (with caveat)** | 起草 → Proposed → 2026-05-11 Accepted with caveat；**W8.2 ORCID 真 OAuth 集成**（better-auth genericOAuth 复用，env 未配 ghost-disabled 替代 PROVIDER_CONFIG_NOT_FOUND）+ Login/Signup Design.md §6.5 重 styling（单 column 400px + ORCID primary CTA + specimen quote aside + md breakpoint）+ 18 OAuth 测试（fakeOrcidTokenExchange）+ §7.1 review log；caveat: 真 sandbox.orcid.org client 端到端 OAuth 回跳 + JWS detached signature 留 Phase 5 Wave B claim-review.sign 路径；review 一等对象表 + 三 capability（review.submit / publish / sign）落到 ADR-0016 Claim-on-Claim Review |
| 0016 | Claim-on-Claim Review（计划） | **Drafting permitted** | improvement-plan §三 Wave B 5 年差异化锚点；reviewer = annotation on claim 的 ORCID-signed provenance lineage；新增第 7 类 maintenance finding `unverified-claim`；Phase 5 Wave B kickoff 前 1 周起草（**新 ADR moratorium 例外**） |
