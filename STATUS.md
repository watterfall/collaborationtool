...
- W4 (ADR-0011): maintenance scan worker — **6 finding 生成器全部交付**（5 SQL-pure: unsupported-claim / outdated-source / unverified-ai-block / contradicted-conclusion / duplicated-claim + 1 网络: broken-citation 经注入式 DoiResolver / `httpDoiResolver` 走 doi.org HEAD）+ apps/agent-worker 路由 maintenance-scan job kind + WorkerConfig.doiResolver 测试注入 hook + 21 单元测试（14 finding + 7 doi-resolver）
- 起草 ADR-0014 / ADR-0015：subdocument 章节级拆分（W5-W6）+ 开放同行评审 ORCID（W8）

**Phase 4 推迟项**：W1 dogfood gate（bwrap 真启动 require Linux host）+ W2 dogfood gate（4 endpoint round-trip require API key）+ W3 端到端真 multi-agent 跑通 + W4 maintenance scan dashboard UI（schema + 6 generator 全交付，UI 推 Phase 4 末 dogfood）+ W5-W6 Yjs subdocument 实施 + W7 章节 fork-merge UI + W8 ORCID 真集成 + W9 跨设备同步 + W10 Loro / Automerge 3 review。
